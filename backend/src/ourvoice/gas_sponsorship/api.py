...
FORUM_WRITE_SELECTORS = {
    ...
    _selector("adjustSupport((uint256,int256,uint8)[])"): "adjustSupport(SupportAdjustment[])",
}
...
def _sponsorship_decision(user_operation: dict[str, Any]) -> tuple[bool, str]:
    ...
    if target in forum_addresses:
        if inner_selector in FORUM_WRITE_SELECTORS:
            return True, f"forum.{FORUM_WRITE_SELECTORS[inner_selector]}"
        ...
        if calls is None:
            return False, "forum multicall could not be decoded"
        if not calls:
            return False, "empty forum multicall is not sponsored"

        call_names: list[str] = []
        for call in calls:
            if len(call) < 4:
                return False, "nested multicall item is missing a selector"
            call_selector = call[:4].hex()
            call_name = FORUM_WRITE_SELECTORS.get(call_selector)
            if call_name is None:
                return False, f"nested forum selector 0x{call_selector} is not sponsored"
            call_names.append(call_name)

        return True, "forum.multicall(" + ",".join(call_names) + ")"
