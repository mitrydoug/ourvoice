...
1. **Indexer** — Subscribes to blockchain events (`StatementAdded`) and writes documents to Meilisearch.
...
| `FORUM_CONTRACT_ADDRESS` | Yes      | —                       | Forum smart contract address |
...
2. **Indexer** — Worker process (override CMD: `python -m ourvoice.main --forum-contract-address ... --ethereum-node-url ... --meili-url ... --meili-api-key ...`)
...
| GET    | `/search` | Full-text search (`?statement_text=...`) |
