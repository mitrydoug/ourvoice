...
contract OurVoiceRegistry is AOurVoiceRegistry {
    ...
    function register(ProofVerificationParams calldata _params) external returns (bytes32) {
        require(devMode || !_params.serviceConfig.devMode, "dev proofs only allowed in dev mode");
        // Verify the proof
        console.log("Verifying proof for user:", msg.sender);
        (bool verified, bytes32 uniqueIdentifier, IZKPassportHelper helper) = zkPassportVerifier.verify(_params);
        ...
        // you specified
        require(
          helper.verifyScopes(_params.proofVerificationData.publicInputs, domain, scope),
          "Invalid scope"
        );
