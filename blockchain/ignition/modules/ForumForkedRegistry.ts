...
export default buildModule("ForumForkedRegistryModule", (m) => {
  ...
  const OurVoiceRegistry = m.contract("OurVoiceRegistry", [MY_SCOPE, "127.0.0.1", ZKPassportVerifier, true]);

  const forums = Object.fromEntries(
    FORUMS.map((forum) => {
      return [
        forum,
        m.contract("Forum", [OurVoiceRegistry, forum == "global" ? "" : forum], { id: `Forum_${forum}` }),
      ]
    })
  );
