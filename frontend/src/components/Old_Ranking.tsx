...
const getUsedCredits = (uncommittedUserVotes: Map<number, number>) => {
  return Array.from(uncommittedUserVotes.values()).reduce(
    (acc, v) => acc + v * v,
    0,
  );
};
...
const submitVotes = async (polyVoice: any, userVotes: Map<number, number>) => {
  const votesArray = Array.from(userVotes.entries()).map(([id, count]) => ({
    statementId: id,
    voteCount: count,
  }));
...
const RankingRow: FC<RankingRowProps> = ({
  index,
  text,
  votes,
  userVotes,
...
const Ranking = () => {
  ...
  const [userVotes, setUserVotes] = React.useState<Map<number, number>>(
    new Map(),
  );

  const [uncommittedUserVotes, setUncommittedUserVotes] = React.useState<
    Map<number, number>
  >(new Map());
    ...
    >
      ...
      >
        ...
        <Stack direction="row" spacing={1} sx={{ marginTop: "0.5rem" }}>
          ...
          <Button
            variant="outlined"
            disabled={userVotes == uncommittedUserVotes}
            onClick={() => {
      ...
      <Box sx={{ width: "100%", padding: "0.5rem" }}>
        <LinearProgress
          variant="determinate"
          value={(1 - usedCredits / USER_CREDIT_BUDGET) * 100}
        />
      </Box>
      <Box sx={{ flexGrow: 1 }}>
        <List>
            ...
            return (
              ...
              >
                <RankingRow
                  ...
                  text={text}
                  votes={votes}
                  userVotes={uncommittedUserVotes.get(Number(id)) || 0}
                  onUserVoteChange={(v) =>
                    setUncommittedUserVotes((m) => {
                      let g = new Map(m);
                      g.set(Number(id), v);
